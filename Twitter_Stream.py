...
class Tweet():
    ...
    # Data on the tweet
    def __init__(self, text, user, date, location):
        self.text = text
        self.user = user
        self.date = date
    ...
    def insertTweet(self):
        ...
        c.execute("INSERT INTO tweets (tweetText, user, date, location) VALUES (?, ?, ?, ?)",
            (self.text, self.user, self.date, self.location))
        conn.commit()
...
if __name__ == '__main__':
    ...
    stream.filter(track=['armed conflict','illegal detention','injustice','activist','warfare','killings','racism', 'racial discrimination','pay inequality','sexual harassment', 'child abuse',
                  ...
                  'Gang violence', 'Child abuse','ethnic cleansing','mass graves','cheap labor', 'gay rights','Forced sterilization','sexual assault','bloodshed','warfare'])
